...
#define KEYBOARD_INPUT_SIZE 32		//键盘输入缓冲区大小
#define MAP_COLS 3			//扫描码列表单个条目的长度
#define NUMBER_SCAN_CODES 0x80		//扫描码条目个数
...
typedef struct {
	...
	char *p_tail;			//指向缓冲区中首个应处理的字节
	int count;			//缓冲区中的字节个数
	char buf[KEYBOARD_INPUT_SIZE];	//缓冲区
}KEYBOARD_INPUT_BUF;


// kernel/keymap.c,所有按键对应的扫描码
extern u32 keymap[NUMBER_SCAN_CODES*MAP_COLS];
